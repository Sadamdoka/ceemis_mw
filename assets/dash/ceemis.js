    ...
    function Init() {
        ...
        function InitIndex() {
            ...
            function signin() {
                ...
                let nin = document.getElementById("txt_nin").value;
                login(nin);
            }
...
// Start recording
            ...
            document.getElementById('btn_high_alert').addEventListener('click', reportHighAlert);
            function reportHighAlert(event) {
                event.preventDefault();
                console.log('High Alert');
                internalCaseReport("0", storedUserData.names, storedUserData.phone, storedUserData.email, storedUserData.passport, storedUserData.userid, storedUserData.jobtype, storedUserData.lcompany, storedUserData.fcompany, getLatitude(), getLongitude(), getAddressName(), "High Emergency Case", "HighAlert");
            }
        ...
        function InitProfile() {
            ...
            $('#puserid').text(storedUserData.userid);
            $('#ppass').val(storedUserData.passport);
            $('#pnin').val(storedUserData.nin);
            $('#pname').val(storedUserData.names);
            $('#ptel').val(storedUserData.ex_phone);
            $('#pemail').val(storedUserData.email);
            $('#pjob').val(storedUserData.jobtype);
            $('#pdob').val(storedUserData.dob);
            $('#paddress').val(storedUserData.location);
        ...
        function login(nin) {
            ...
            $.ajax(settings).done(function (response) {
                ...
                if (obj.status === true) {
                    ...
                    showMessage('Error', "Invalid Passport | Nin No. Please Create Account", 'error');
                    // alert(response.msg);
                }
            });
        }
...
//Who Selecter
                    ...
                    success: function (data) {
                        ...
                        try {
                            ...
                            if (!isEmpty(data)) {
                                ...
                                const userDataString = JSON.stringify(jdata);
        ...
        async function getCase(pass, userid, elm) {
            // Show spinner before AJAX call
            showProgress();
            fetch(sessionStorage.getItem('url') + "fetch/case_ticket_mw/" + pass + "/" + userid)
                    .then(res => res.json())
                    .then(data => {
...
// Helper function to create a message element
        ...
        // Reusable function to render locations on a Leaflet map
        function renderMap(mapId, locations, zoom = 18) {
            ...
            // Filter out invalid entries (missing latitude/longitude)
            ...
            // Initialize map at first valid location
            const firstLoc = validLocations[0];
            const map = L.map(mapId).setView([parseFloat(firstLoc.latitude), parseFloat(firstLoc.longitude)], zoom);
            ...
            validLocations.forEach(loc => {
                ...
                const customIcon = L.icon({
                    iconUrl: loc.pic || 'https://cdn-icons-png.flaticon.com/512/684/684908.png',
                    iconSize: [40, 40],
                    iconAnchor: [20, 40],
                    popupAnchor: [0, -40]
                });
        ...
        async function renderGoogleMap(mapId, locations, defaultZoom = 18) {
            ...
            const validLocations = locations.filter(loc => loc && loc.latitude && loc.longitude && loc.latitude !== "NA" && loc.longitude !== "NA");
            if (validLocations.length === 0) {
                console.warn('No valid locations with latitude/longitude.');
                return;
            }
            ...
            const map = new google.maps.Map(document.getElementById(mapId), {
                zoom: defaultZoom, // initial close-up
            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(
                        position => {
                            ...
                            // Initially center and zoom on user
